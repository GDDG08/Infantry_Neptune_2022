...
/**
  ...
  * @retval     NUL
  */
uint32_t gyro_cont = 0; float gyro_rate;
void BMI055_GyroReadOver(uint8_t *rx_buf) {
    BMI055_BMI055DataTypeDef *bmi055 = BMI055_GetBMI055DataPtr();
    if (gyro_cont == 0) gyro_cont = HAL_GetTick();
    
    gyro_cont++;
    gyro_rate = gyro_cont * 1000 / HAL_GetTick();
    if (bmi055->init_flag != 1) return;
...
/**
  ...
  * @retval     NUL
  */
uint32_t acc_cont = 0; float acc_rate;
void BMI055_AccelReadOver(uint8_t *rx_buf) {
    BMI055_BMI055DataTypeDef *bmi055 = BMI055_GetBMI055DataPtr();
    if (acc_cont == 0) acc_cont = HAL_GetTick();
