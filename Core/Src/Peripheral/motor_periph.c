...
void Motor_InitAllMotors() {
    ...
    Motor_InitMotor(&Motor_shooterMotorLeft, Motor_TYPE_PWM_MOTOR, 1, 0, 0.05, &htim3, TIM_CHANNEL_1, &htim5, shooter_encoder_callback);
    Motor_InitMotor(&Motor_shooterMotorRight, Motor_TYPE_PWM_MOTOR, 1, 0, 0.05, &htim3, TIM_CHANNEL_2, &htim4, shooter_encoder_callback);
...
void Motor_SendMotorPWMOutput(Motor_MotorTypeDef* pmotor) {
    ...
    // float duty = 1000 * 0.00011136f + 0.47522f;
    pmotor->duty = duty;
