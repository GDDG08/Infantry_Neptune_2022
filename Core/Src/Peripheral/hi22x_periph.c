...
UART_HandleTypeDef* Const_HI22X_UART_HANDLER         = &huart3;
...
//     if (CRC_HI22XEnum == NOT_MATCH) return;
    ...
    if (hi22x->now_angle.pitch - hi22x->last_angle.pitch < -181)	hi22x->count.pitch++;
    if (hi22x->now_angle.pitch - hi22x->last_angle.pitch >  181)	hi22x->count.pitch--;
    hi22x->angle.pitch = (float)hi22x->count.pitch * 360.0f + hi22x->now_angle.pitch;

    if (hi22x->now_angle.yaw - hi22x->last_angle.yaw < -181)	    hi22x->count.yaw++;
    if (hi22x->now_angle.yaw - hi22x->last_angle.yaw >  181)	    hi22x->count.yaw--;
    hi22x->angle.yaw = (float)hi22x->count.yaw * 360.0f + hi22x->now_angle.yaw;

    if (hi22x->now_angle.row - hi22x->last_angle.row < -181)	    hi22x->count.row++;
    if (hi22x->now_angle.row - hi22x->last_angle.row >  181)	    hi22x->count.row--;
...
void HI22X_RXCallback(UART_HandleTypeDef* huart) {
    ...
    int rxdatalen = Const_HI22X_RX_BUFF_LEN - Uart_DMACurrentDataCounter(huart->hdmarx->Instance);
